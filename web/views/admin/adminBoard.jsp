				...
				<div class="card-body">
					<table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
						...
						<tbody id="selectTd">
							...
							%>
							<tr>
								<input type="hidden" value="<%=s.getCid()%>" id="seller<%=s.getCid() %>" name="seller<%=s.getCid()%>">
								<td><%=s.getSid()%></td>
								<td><%=s.getbName()%></td>
								<td><%=s.getbNum()%></td>
								<td><%=s.getAcName()%></td>
					...
					<div>
						...
						<li class="page-item"><a class="page-link" href="<%=request.getContextPath()%>/seInfo.ad?currentPage=1">FirstPage</a></li>
						
						<!-- 이전 페이지로 -->
						<% if (currentPage <= 1) { %>
							<li class="page-item"><a class="page-link" href="#">Previous</a></li>
						<% } else { %>
							<li class="page-item"><a class="page-link" href="<%=request.getContextPath()%>/seInfo.ad?currentPage=<%=currentPage - 1%>">Previous</a></li>
						<%} %>
						<!-- 10개의 페이지 목록 -->
						<% for (int p = startPage; p <= endPage; p++) { %>
							...
							<%} else if(p == endPage && p<(startPage+9) ){%>
								...
								<%} %>
							<% } else { %>
								<li class="page-item"><a class="page-link" href="<%=request.getContextPath()%>/seInfo.ad?currentPage=<%=p%>"><%=p%></a></li>
							<%} %>
						<%} %>
						<!-- 다음 페이지로(>) -->
						...
						<% } else { %>
							<li class="page-item"><a class="page-link" href="<%=request.getContextPath()%>/seInfo.ad?currentPage=<%=currentPage + 1%>">Next</a></li>
						<% } %>	
						<!-- 처음으로 -->
						<li class="page-item"><a class="page-link" href="<%=request.getContextPath()%>/seInfo.ad?currentPage=<%=maxPage%>">LastPage</a></li>
						</ul>
					</div>
				</div>
			</div>
