...
<%-- <%@ include file="../common/menubar.jsp" %> --%>
	...
	<div class="outer">
		...
		<div class="tableArea">
			<table id="productTable" align="center" width="90%" border="1px solid black" style="border-collapse:collapse">
				...
				<tr>
					<td rowspan="1">총 수익 : <%=pPrice %></td>
				</tr>
				<tr>
					<td rowspan="1">판매 수량 : <%=pPrice %></td>
				</tr>
				<tr>
					<td rowspan="1">판매 시작일 : <%=createDate %> ~ <%=pPeriod %></td>
				</tr>
				<tr>
					<td >
						상품소개 : <%=pContent %>
		 ...
		 $(window).scroll(function(){	//스크롤 이벤트 
			if($(window).scrollTop() == $(document).height() - $(window).height()){	//스크롤바 바닥까지 내려가면 내용 추가
				...
				$(".tableArea").append("<hr><table id='productTable' align='center' width='90%' border='1px solid black' style='border-collapse:collapse'>"
						...
						+ "<td rowspan='1' width='65%'>상품명 : " + pName + "</td></tr>"
						+ "<tr><td rowspan='1'>총 수익 : " +pPrice+"</td></tr>"
						+ "<tr><td rowspan='1'>판매 수량 : " +pPrice+"</td></tr>"
						+ "<tr><td rowspan='1'>판매 시작일 : " + createDate +"~"+ pPeriod +"</td></tr>"
	...
	/* 
	$(document).ready(function(){
		createGraph("#data-table", "#chart");	//#data-table : 데이터 값이 입력되어있는 테이블 id , #chart : 차트를 그릴 공간의 id(div의 id)
	...
	https://mytory.net/archives/1893
	*/
